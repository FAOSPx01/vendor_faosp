...
func init() {
	android.RegisterModuleType("faosp_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = faospExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = faospExpandVariables(ctx, dep_file)
	...
	cmd := faospExpandVariables(ctx, String(g.properties.Cmd))
