...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^faosp_") ; then
        FAOSP_BUILD=$(echo -n $1 | sed -e 's/^faosp_//g')
    else
        FAOSP_BUILD=
    fi
    export FAOSP_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/faosp/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the FAOSP model name
            if [ -z "$variant" ]; then
                variant="user"
            fi

            lunch faosp_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/FAOSP-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.faosp.device | grep -q "$FAOSP_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $FAOSP_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/FAOSPx01/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.faosp.device | grep -q "$FAOSP_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.faosp.device | grep -q "$FAOSP_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $FAOSP_FIXUP_COMMON_OUT ]; then
